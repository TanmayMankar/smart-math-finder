  ...
  const handleExtractMath = async () => {
    ...
    try {
      const response = await axios.post(
        'http://localhost:8080/api/pdf/extract-math',
        formData,
        {
          headers: {
      ...
      if (requestError.response) {
        setError('Could not extract expressions. Please verify your PDF and try again.')
      } else {
        setError('Server unavailable. Please make sure the backend is running on port 8080.')
      }
      setExpressions([])
    } finally {
      setLoading(false)
      ...
      <div className="mx-auto w-full max-w-4xl">
        <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm sm:p-8">
          ...
          <p className="mt-2 text-sm text-slate-600 sm:text-base">
            Upload a PDF and extract all detected mathematical expressions instantly.
          </p>
        ...
        <section className="mt-6 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm sm:p-8">
          <h2 className="text-lg font-semibold text-slate-900 sm:text-xl">Extracted Expressions</h2>
          ...
          {expressions.length > 0 && (
            <div className="mt-4 grid gap-3 sm:grid-cols-2">
              {expressions.map((expression, index) => (
                ...
                >
                  <p className="break-words font-mono text-sm text-slate-800">{expression}</p>
                </div>
              ))}
            </div>
          )}
